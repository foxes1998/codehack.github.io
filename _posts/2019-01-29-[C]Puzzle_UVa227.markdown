...
此段为读取所有输入，将其存入字符数组 s （过滤掉换行符），其中用 tmp 字符数组读取每行内容，再加入数组 s 。其中` scanf("%[^\n]%*c",tmp); ` 效果等价于` gets(); ` ，仅作知识补充。若输入为上面的 示例 输入，则输出 s 为： `TRGSJXDOKIM VLNWPABEUQHCFARRBBL0ABCDEFGHIJKLMNOPQRS TUVWXAAALLLL0ABCDEFGHIJKLMNOPQRS TUVWXAAAAABBRRRLL0Z` 。
